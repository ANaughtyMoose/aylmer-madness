...
| 11 | **Save mid-job, then load** | ~~The slot does not carry a job in progress. Loading dropped « Ramasser la gang » at stage 1 and put the player back in free roam **with no warning either way**.~~ **Fixed on `wave/2b-saves`.** The slot carries `mission.def.id`, the stage index, the clock, the passengers and whatever scalars the running stage was counting; loading restarts that stage and says « Job reprise: … ». Verified in Chrome: saved « Première période » at stage 0 with 418.0 s left, reloaded the page, pressed Continuer, resumed at stage 0 with 417.9 s (`docs/shots/save-resume.jpg`). A def that has been pulled out of the build says « Job perdue » instead of dropping you silently. | annoying | fixed |
