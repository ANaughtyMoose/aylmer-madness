...
| # | Item | Pri | Eff | Notes |
|---|------|-----|-----|-------|
| D1 | Wheels don't turn with steering on the skinned model until the loft is re-lofted | P2 | S | Visible on `garage.html`; also front wheels should steer in the garage. |
...
| D6 | Gamepad mapping untested on a real pad | P2 | S | Verify Xbox/PS layouts in Chrome, add rumble on impact. |
...
| W7 | Night: streetlights don't light anything | P2 | M | Add a few point lights to the shader (nearest 8 lamps) or fake light pools (unlit discs on the road under each lamp). Headlight cones for the player at night. |
...
| # | Item | Pri | Eff | Notes |
|---|------|-----|-----|-------|
| G1 | Mission timers never tuned by a real player | P1 | S | Drive each job once, set timers at ~1.3× my time. `tour` checkpoints and `dep` are the tight ones. |
...
| G3 | More jobs | P2 | M | Ideas: drive-in at the Aylmer cinema, Sunday hockey at the arena, the ferry-less run to Quyon, snow tires at Canadian Tire, "return the Sunfire before Dave's dad gets home". |
...
| T2 | Chunk meshes are one draw each (~100 draws/frame) | P3 | M | Merge into fewer VAOs per quality tier; not needed unless T1 says so. |
| T3 | `mapdata.js` is 3.2 MB | P2 | S | Gzip via the server, or split water/areas into a lazy chunk; quantise coords to int16. |
