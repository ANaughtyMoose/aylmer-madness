...
| ~~G3~~ | ✅ More jobs | P2 | M | Ideas: drive-in at the Aylmer cinema, Sunday hockey at the arena, the ferry-less run to Quyon, snow tires at Canadian Tire, "return the Sunfire before Adam's dad gets home". **Done:** three side jobs shipped (canoe, Sayyad, couch); the other ideas remain **Done: three side jobs + four races.** |
...
| ~~U9~~ | ✅ **Remove the second click to start** (Thomas, 2026-09-08: « one click starts ») | P1 | S | The picker stays — it is where the character and the start point are chosen — but nothing on it asks for a confirm any more. The GO bar names the point in full (« GO ▸ Chez nous, 299 Chemin Fraser »), and a click on a start point you have already picked, in the list or on the map, IS the go; a first click still just previews it. `goFromPicker()` is the single path in `main.js`, `tools/smoke_shell.mjs` pins it, and `#start` / `#startconfirm` are untouched so `tools/headless.mjs` still boots the game. |
...
| ~~C6~~ | ✅ **In-car view: the driver's seat** (Thomas, 2026-09-06; shipped 2026-09-07) | P1 | M | A fifth `C` stop, `driver`, with the eye inside the cab. **Not** the interior plate this row asked for: a plate is a second renderer with its own baked sun, and the brief also asks that the render stay consistent when you press `C` back to chase. The cab is a mesh in the engine's own flat-shaded language (`src/game/cockpit.js`, 2510 tris for the whole Ranger cab, wheel, cluster and exterior details), drawn with the car's own model matrix, so it pitches and rolls with the body and the sun that lights the hood lights the dash. Everything on the cab is derived from the spec's own `glassTop` / `top` / `plan`, so every car in `CARS` gets one; the Ranger gets Tom's verified checklist (`gemini-inbox/interiors/RANGER-HANDOFF.md` §2) on top. Head dip on the brakes, idle shiver, Shift turns the head, and the radio takes a 2.4 kHz lowpass from the seat. `tools/smoke_cockpit.mjs`, four shots in `docs/shots/cockpit-ranger-*.jpg`. **Still open:** the per-car details this row lists (the Civic's red cluster, Mike's Bluesfest wristband, the Saturn's plastic) — every non-Ranger gets the generic cab. |
